from alt_lk import BBox, LatLng

PLACE_BBOX_IDX = {
    'Sri Lanka': BBox(
        LatLng(5.91, 79.65),
        LatLng(9.84, 82.19),
    ),
    'Nuwara Eliya': BBox(
        LatLng(6.902851424723703, 80.58878931660789),  
        LatLng(7.112105098302233, 80.92204999330954),  
    ),
    'Horton Plains': BBox(
        LatLng(6.666381112559362, 80.70465850712198),
        LatLng(6.903176670504809, 80.9081260851063),
    ),
     'Sri Paada': BBox(
        LatLng(6.64778323363658, 80.34894180132841), 
        LatLng(6.777296653740274, 80.46194298883222),
    ),
    'Kandy': BBox(
        LatLng(7.263741575093108, 80.58389833079025),
        LatLng(7.332285244984741, 80.67434703330403),
    ),
    'Colombo': BBox(
        LatLng(6.880078017904935, 79.82096863616397),
        LatLng(6.958174410944311, 79.94629291844821),
    ),
    'Viharamahadevi Park': BBox(
        LatLng(6.912032282921783, 79.85572282625117),
        LatLng(6.916671622967184, 79.86473551211941),
    ),
}
